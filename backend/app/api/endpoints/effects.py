...
from app.models.effects import EffectConfig, TransitionConfig
...
async def apply_filter(
    ...
    try:
        ...
        return {"status": "success", "message": f"Successfully applied visual filter: {filter_name}"}
...
async def apply_transition(
    ...
    """
    try:
        effect_engine.apply_transition(video1_path, video2_path, transition_name, duration, offset, output_path)
        return {"status": "success", "message": f"Successfully applied transition: {transition_name}"}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
