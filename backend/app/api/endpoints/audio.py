...
async def normalize_audio(request: Request, audio_path: str, output_path: str, target_db: float = -16.0):
    """
    ...
    try:
        ...
        return {"status": "success", "message": "Audio loudness normalized successfully under EBU R128."}
...
async def mix_audio(request: Request, track_paths: List[str], output_path: str):
    ...
    try:
        ...
        return {"status": "success", "message": f"Successfully mixed {len(track_paths)} audio channels."}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@limiter.limit("10/minute")
async def compress_audio(request: Request, audio_path: str, output_path: str, threshold: float = -20.0, ratio: float = 4.0):
    """
    ...
    try:
        ...
        return {"status": "success", "message": "Dynamic audio compressor applied successfully."}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def reduce_noise(request: Request, audio_path: str, output_path: str):
    ...
    try:
        ...
        return {"status": "success", "message": "Audio noise reduction filter applied successfully."}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
