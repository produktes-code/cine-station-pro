...
async def create_job(request: Request, payload: RenderJobPayload):
    """
    Registers a new rendering job task entry in queued state.
    """
    try:
        job_id = render_manager.create_render_job(
            payload.project_id, payload.timeline_data, payload.export_config
        )
...
@limiter.limit("5/minute")
async def start_job(request: Request, job_id: str, payload: RenderJobPayload):
    """
    Starts rendering an enqueued job, subject to the maximum concurrency limit of 3.
    """
    started = render_manager.start_render(
        job_id, payload.project_id, payload.timeline_data, payload.export_config
    )
    if not started:
        raise HTTPException(
...
async def get_progress(request: Request, job_id: str):
    ...
    return {"job_id": job_id, "progress": progress, "status": status_str}
