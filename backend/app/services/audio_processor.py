...
class AudioProcessor:
    ...
    def _run_ffmpeg_cmd(self, cmd: List[str], desc: str) -> None:
        ...
        try:
            ...
            if process.returncode != 0:
                logger.error(
                    f"FFmpeg Audio error ({desc}). Return code: {process.returncode}. Stderr: {process.stderr}"
                )
                raise RuntimeError(
                    f"FFmpeg audio command failed: {process.stderr.strip()}"
                )
            logger.info(f"FFmpeg Audio completed: {desc}")
        except subprocess.TimeoutExpired as e:
            logger.error(
                f"FFmpeg Audio command timed out ({desc}) after {settings.RENDER_TIMEOUT} seconds."
            )
    ...
    def normalize_audio(
        self, audio_path: str, output_path: str, target_db: float = -16.0
    ) -> None:
        """
    ...
    def mix_audio_tracks(self, track_paths: List[str], output_path: str) -> None:
        ...
        cmd += [
            "-filter_complex",
            f"amix=inputs={len(track_paths)}:duration=longest:dropout_transition=2",
            output_path,
        ]
        self._run_ffmpeg_cmd(cmd, f"Mix {len(track_paths)} audio tracks")

    def apply_eq(
        self, audio_path: str, output_path: str, bands: Dict[int, float]
    ) -> None:
        """
    ...
    def apply_compression(
        ...
        cmd = [
            ...
            audio_path,
            "-af",
            f"acompressor=threshold={threshold}:ratio={ratio}:attack=20:release=250",
            output_path,
        ]
        self._run_ffmpeg_cmd(
            cmd, f"Apply compressor (threshold={threshold}dB, ratio={ratio}:1)"
        )
