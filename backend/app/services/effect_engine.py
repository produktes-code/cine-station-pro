...
class EffectEngine:
    ...
    def _run_ffmpeg_cmd(self, cmd: List[str], desc: str) -> None:
        ...
        try:
            ...
            if process.returncode != 0:
                logger.error(f"FFmpeg Effect error ({desc}). Return code: {process.returncode}. Stderr: {process.stderr}")
                raise RuntimeError(f"FFmpeg effect engine failed: {process.stderr.strip()}")
            logger.info(f"FFmpeg Effect completed: {desc}")
        except subprocess.TimeoutExpired as e:
            logger.error(f"FFmpeg Effect command timed out ({desc}) after {settings.RENDER_TIMEOUT} seconds.")
            raise TimeoutError(f"FFmpeg effect command timed out: {e}")
        except Exception as e:
            logger.exception(f"Exception executing Effect command: {e}")
            raise

    def apply_filter(self, video_path: str, filter_name: str, output_path: str, intensity: float = 1.0) -> None:
        """
        ...
        self._run_ffmpeg_cmd(cmd, f"Apply filter '{filter_name}' (intensity={intensity})")

    def apply_transition(self, video1_path: str, video2_path: str, transition_name: str, duration: float, offset: float, output_path: str) -> None:
        """
        ...
        self._run_ffmpeg_cmd(cmd, f"Transition '{transition_name}' (d={duration}s, offset={offset}s)")

    def apply_transformation(self, video_path: str, output_path: str, scale: str = None, rotation: float = None, crop: str = None, flip: str = None) -> None:
        """
        ...
        self._run_ffmpeg_cmd(cmd, f"Apply transformations (scale={scale}, rotation={rotation}, flip={flip})")

    def apply_keying(self, video_path: str, output_path: str, key_type: str = "chroma", color: str = "0x00FF00", similarity: float = 0.2, blend: float = 0.1) -> None:
        """
    ...
    def apply_keyframes(self, video_path: str, output_path: str, parameter: str, keyframes: List[Dict[str, Any]]) -> None:
        """
        ...
        if parameter == "brightness":
            ...
            self._run_ffmpeg_cmd(cmd, f"Apply keyframes for brightness: {len(keyframes)} keys")
        elif parameter == "zoom":
            ...
            self._run_ffmpeg_cmd(cmd, "Apply keyframes for zoom (progressive scale zoompan)")
        else:
