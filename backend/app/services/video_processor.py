...
class VideoProcessor:
    ...
    def _run_ffmpeg_cmd(self, cmd: List[str], desc: str) -> None:
        ...
        try:
            ...
            if process.returncode != 0:
                logger.error(
                    f"FFmpeg error ({desc}). Return code: {process.returncode}. Stderr: {process.stderr}"
                )
                raise RuntimeError(f"FFmpeg command failed: {process.stderr.strip()}")
            logger.info(f"FFmpeg completed successfully: {desc}")
        except subprocess.TimeoutExpired as e:
            logger.error(
                f"FFmpeg command timed out ({desc}) after {settings.RENDER_TIMEOUT} seconds."
            )
    ...
    def get_video_info(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if process.returncode != 0:
                ...
                raise RuntimeError(
                    f"ffprobe metadata extraction failed: {process.stderr.strip()}"
                )
    ...
    def validate_video_format(self, file_path: str) -> None:
        ...
        if not valid:
            ...
            raise ValueError(
                f"No supported video codecs found. Found: {codecs_found}. Supported: {supported_codecs}"
            )
    ...
    def concat_videos(self, input_paths: List[str], output_path: str) -> None:
        ...
        list_file_path = os.path.join(
            settings.TEMP_DIR, f"concat_list_{os.getpid()}.txt"
        )
        try:
    ...
    def create_proxy(
        self, video_path: str, output_path: str, resolution: str = "480p"
    ) -> None:
        """
        ...
        for s in info.get("streams", []):
            if s.get("codec_type") == "video":
                ...
                if (
                    ...
                    or "smpte2084" in color_transfer.lower()
                    or "arib-std-b67" in color_transfer.lower()
                ):
                    is_hdr = True
                break
