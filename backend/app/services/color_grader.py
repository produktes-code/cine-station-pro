...
class ColorGrader:
    ...
    def _run_ffmpeg_cmd(self, cmd: List[str], desc: str) -> None:
        ...
        try:
            ...
            if process.returncode != 0:
                logger.error(
                    f"FFmpeg Color Grading error ({desc}). Return code: {process.returncode}. Stderr: {process.stderr}"
                )
                raise RuntimeError(
                    f"FFmpeg color grading failed: {process.stderr.strip()}"
                )
            logger.info(f"FFmpeg Color Grading completed: {desc}")
        except subprocess.TimeoutExpired as e:
            logger.error(
                f"FFmpeg Color Grading timed out ({desc}) after {settings.RENDER_TIMEOUT} seconds."
            )
    ...
    def adjust_lift_gamma_gain(
        self, video_path: str, lift: float, gamma: float, gain: float, output_path: str
    ) -> None:
        """
        ...
        cmd = [
            ...
            video_path,
            "-vf",
            f"eq=brightness={lift}:gamma={gamma}:contrast={gain}",
            "-c:v",
            "libx264",
            "-c:a",
            "copy",
        ...
        self._run_ffmpeg_cmd(
            cmd, f"Adjust Lift (B={lift}), Gamma (G={gamma}), Gain (C={gain})"
        )

    def adjust_saturation(
        self, video_path: str, saturation: float, output_path: str
    ) -> None:
        """
    ...
    def adjust_contrast(
        self, video_path: str, contrast: float, output_path: str
    ) -> None:
        """
    ...
    def apply_preset(self, video_path: str, preset_name: str, output_path: str) -> None:
        ...
        if preset_name not in presets:
            logger.error(
                f"Preset '{preset_name}' is not defined. Defaulting to bypass."
            )
            cmd = [
                settings.FFMPEG_PATH,
                "-y",
            ...
            self._run_ffmpeg_cmd(
                cmd, f"Color grade bypass (Invalid preset '{preset_name}')"
            )
            return
