...
class MediaManager:
    ...
    def generate_thumbnails(self, video_path: str, count: int = 5) -> List[str]:
        ...
        for i in range(1, count + 1):
            ...
            try:
                ...
                logger.warning(f"Could not extract thumbnail at timestamp {timestamp}s: {e}")
    ...
    def detect_scenes(self, video_path: str) -> List[float]:
        ...
        try:
            ...
            logger.info(f"Scene detection completed. Detected {len(scene_cuts)} cuts: {scene_cuts}")
            return scene_cuts
        except Exception as e:
