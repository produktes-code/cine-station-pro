...
class TestFFmpegCommandConstruction:
    ...
    def test_concat_videos_command_structure(self):
        ...
        with (
            patch.object(self.processor, "_run_ffmpeg_cmd") as mock_run,
...
class TestGetVideoInfo:
    ...
    def test_get_video_info_ffprobe_failure_raises(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            with pytest.raises(
                RuntimeError, match="ffprobe metadata extraction failed"
            ):
...
class TestConcatTempFileCleanup:
    ...
    def test_temp_file_removed_after_concat(self):
        ...
        with (
            patch.object(self.processor, "_run_ffmpeg_cmd"),
            patch("builtins.open", MagicMock()),
            patch("os.path.exists", return_value=True),
    ...
    def test_temp_file_cleanup_on_error(self):
        ...
        with (
            patch.object(
                self.processor, "_run_ffmpeg_cmd", side_effect=RuntimeError("fail")
            ),
            patch("builtins.open", MagicMock()),
            patch("os.path.exists", return_value=True),
            patch("os.remove") as mock_remove,
